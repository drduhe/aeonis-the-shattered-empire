...
To see how they fit together, follow one example round at a four-player table. Our leads: **Rakhis, the Sandlord** (fast cavalry raider; 2 VP, 1 Renown) and **Vharok, Lord of Steel** (fortress tank; 3 VP, 5 Renown). Cassian and Elyndra fill the other seats. It is Round 3; Cassian holds the Speaker token.
...
> **Example:** Rakhis moves a Cavalry group 2 hexes toward Vharok's line. The first hex, a Plains adjacent to Vharok's Infantry: 1 AP terrain, ZOC surcharge waived (flanking). The second Plains hex, also in ZOC: 1 + 1 = 2 AP. Total: **3 AP**. An Infantry group would have paid 2 AP for the first hex — and stopped there (range 1).
