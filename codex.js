...
function getRuntimeConfig() {
  ...
  return {
    goatcounter: typeof w.AEONIS_GOATCOUNTER === "string" ? w.AEONIS_GOATCOUNTER.trim() : ""
  };
}
...
async function openDoc(path, { scrollIntoView = false } = {}) {
  const manifestDoc = state.docByPath.get(path);
  const doc = manifestDoc || docStub(path);
...
function wireEvents() {
  ...
  function syncNavActive() {
    ...
    desktopLinks.forEach((a) => {
      ...
      a.classList.toggle("is-active", href.includes("codex.html"));
    });
  }
...
function initGoatCounter() {
  const code = getRuntimeConfig().goatcounter;
  if (!code) return;
